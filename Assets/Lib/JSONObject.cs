...
public class JSONObject  {
	...
	public JSONObject(Dictionary<string, string> dict){
		type = Type.OBJECT;
		foreach(var val in dict){
			this.AddField(val.Key,val.Value);
		}
	}
	...
	public void AddField(string name, JSONObject obj) {
		if(obj != null){		//Don't do anything if the object is null
			if(type != JSONObject.Type.OBJECT){
				...
				Debug.LogWarning("tried to add a field to a non-object JSONObject.  We'll do it for you, but you might be doing something wrong. " + obj.str);
			}
			keys.Add(name);
			list.Add(obj);
		}
	}
	public void SetField(string name, string obj) {
		if(HasField(name)) {
